...
const AWARDS = [
  { place:'2nd place', prize:4000, name:'Yconic × NVIDIA Hackathon', accent:'fw',
    d:'Retail floor de-escalation — ESP32 audio, local transcription and sentiment, and a dashboard that flags escalating lanes. Audio never leaves the device.' },
...
const GROUPS = [
  ...
  { id:'wins',     label:'Built under pressure',
    note:'Hackathon builds, all three second place. Two are public team repositories; the third is private. Designed, built and demoed inside a weekend each, which is visible in the code and worth saying rather than hiding.' },
...
const PROJECTS = [
  ...
  { g:'systems', cover:'flow', slug:'inhabit-data-pipeline', title:'Inhabit Data Pipeline', when:'2026',
  ...
  { g:'wins', cover:'thermal', slug:'datacenter-thermal-twin', title:'Datacenter Thermal Twin', when:'TVG Hackathon · 2nd, $3,000',
    stack:'Leaflet · OSRM · Express · Firebase · Python thermal model',
    d:'A district-heating planner. Pick a building that needs heat and it routes waste heat to it from a datacenter, a transit tunnel or an industrial site — along real road geometry from OSRM, around blockages you paint on the map. The browser posts routing commands once a second, a Python thermal model applies them and posts telemetry back. I wrote the 854-line facility-location optimiser that decides where the storage tanks should go, plus the auth layer and the API server.',
    stats:[['placed','2nd'],['prize','$3,000'],['built in','one weekend']],
    links:[['HeatRouter — team repo','https://github.com/lukelul/HeatSync-TVG-Hackathon-Winner-']] },

  { g:'wins', cover:'chart', slug:'escalation-sentiment-dashboard', title:'Escalation Sentiment Dashboard', when:'Yconic × NVIDIA · 2nd, $4,000',
    stack:'Flask · faster-whisper · RoBERTa · SQLite · ESP32 I²S',
    d:'Retail floor de-escalation. An ESP32 with a MAX4466 mic streams audio over I²S, and a dashboard flags the checkout lanes that are heading sideways before anyone asks for a manager. Everything runs locally — faster-whisper for transcription, a RoBERTa sentiment model on top — so the audio never leaves the machine. The privacy control is geographic: recording is refused outright in break rooms, stockrooms and manager offices, and every refusal is logged and counted rather than silently dropped.',
  ...
  { g:'nda', cover:'stack', slug:'design-review-and-benchmarking', title:'Design Review &amp; Benchmarking', when:'2026 · GE Vernova',
  ...
  { g:'craft', cover:'chart', slug:'finabulary', title:'Finabulary', when:'2026',
  ...
  { g:'craft', cover:'stack', slug:'virtual-wardrobe-platform', title:'Virtual Wardrobe Platform', when:'2026',
  ...
  { g:'roots', cover:'net', slug:'foundations', title:'Foundations', when:'2024 – 2025',
